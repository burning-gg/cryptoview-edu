      ...
      <Row className='chart-header'>
        ...
        <Col className='price-container'>
          ...
          <Title level={5} className='current-price'>
            Current {cryptoDetails.name} Price: $ {millify(cryptoDetails.price)}
          </Title>
        </Col>
      </Row>
      <Row className='chart-options-container'>
        <Select
          className='select-timeperiod'
          placeholder='Select Time Period'
          onChange={(value) => onChangeTimePeriod(value)}
        >
          {time.map((date) => (
            <Option key={date}>{date}</Option>
          ))}
        </Select>
        <Button type={!isZero ? 'default' : 'primary'} onClick={onToggleIsZero}>
          Begin at zero
        </Button>
      </Row>
